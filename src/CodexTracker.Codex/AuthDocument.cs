    ...
    DateTimeOffset? AccessTokenExpiresAt = null)
{
    public static AuthDocument Parse(byte[] json)
    {
        ...
        return new(email, accountId, access, plan, startedAt, endsAt, ReadExpiry(accessClaims.RootElement));
    }

    private static DateTimeOffset? ReadExpiry(JsonElement claims)
    {
        if (!claims.TryGetProperty("exp", out var expiry) || expiry.ValueKind != JsonValueKind.Number ||
            !expiry.TryGetInt64(out var unixSeconds)) return null;
...
public sealed class TrackerException(string message, CodexFailureCode code = CodexFailureCode.Unknown) : Exception(message)
{
    public CodexFailureCode Code { get; } = code;
}
